...
public class RunNoIncrementTest {
    ...
    void testRunNoIncrementsSequentially() {
        ...
        // Verify runs are saved with correct run_no values
        List<PipelineRunEntity> runs = pipelineRunRepository
                .findByPipelineNameOrderByRunNoAsc(pipelineName);

        assertEquals(3, runs.size(), "Should have 3 pipeline runs");

        // Verify run_no increments: 1, 2, 3
        assertEquals(1, runs.get(0).getRunNo(), "First run should have run_no=1");
        assertEquals(2, runs.get(1).getRunNo(), "Second run should have run_no=2");
        assertEquals(3, runs.get(2).getRunNo(), "Third run should have run_no=3");
    ...
    void testGetNextRunNoForPipeline() {
        ...
        List<PipelineRunEntity> runs = pipelineRunRepository
                .findByPipelineNameOrderByRunNoAsc(pipelineName);
        assertTrue(runs.isEmpty(), "Should have no runs initially");
        ...
        // Check we have 1 run
        runs = pipelineRunRepository.findByPipelineNameOrderByRunNoAsc(pipelineName);
        ...
        // Check we have 2 runs
        runs = pipelineRunRepository.findByPipelineNameOrderByRunNoAsc(pipelineName);
        assertEquals(2, runs.size());
        assertEquals(2, runs.get(1).getRunNo(), "Second run should have run_no=2");

        // Next run should be 3
    ...
    void testRunNoDifferentPipelines() {
        // Different pipelines should have independent run_no sequences
        ...
        runB1.setRunNo(1);  // Should start at 1 for different pipeline
        ...
        // Verify Pipeline A has run_no 1, 2
        ...
        // Verify Pipeline B has run_no 1
