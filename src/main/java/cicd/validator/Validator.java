...
public class Validator {
    ...
    private void checkJobs() {
        ...
        for (Job j : p.jobs.values()) {
            if (j.stage != null && !p.stages.contains(j.stage)) {
                errors.add(file + ":" + j.line + ":" + j.col + ": job `" + j.name + "` references undefined stage `" + j.stage + "`");
            }
        }
    }
    ...
    private boolean hasCycle(String node, Map<String, List<String>> graph, Set<String> visited, List<String> path) {
